...
Em certos casos, os usuários receberão um conjunto especial de credenciais que poderão importar bancos de dados PipeLine se desejarem configurar seu programa desta forma (consulte Configurando um novo programa no Anexo 1).

## Etapa 1: Converta o arquivo .accdb em um arquivo JSON:

*Sobre a ferramenta de conversão PipeLine Access DB para JSON:* Como o sistema QAT só é capaz de importar dados do programa no formato JSON e os bancos de dados PipeLine estão no formato .accdb, o QAT tem uma ferramenta para download para converter o arquivo .accdb em um arquivo JSON, permitindo assim a importação para o QAT. Uma vez instalada, esta ferramenta não requer acesso à Internet para funcionar. Além disso, esta ferramenta permite ao usuário escolher o idioma de sua preferência.

1. Confirme se você possui os requisitos de configuração do sistema apropriados:
   - Sistema operacional: *Windows/Linux/Ubuntu*
   - Outros softwares: *Java JDK 1.8 e superior.*
2. Confirme se os dados do PipeLine estão totalmente atualizados:
...
9. Clique no segundo botão Procurar para selecionar o local onde o arquivo convertido será armazenado na máquina local do usuário.
10. Clique em Converter. Quando o arquivo for convertido para o formato .json, você receberá a notificação abaixo:
...
11. No QAT, vá para o item de menu "Gerenciamento de Programas" &gt; "Importação de Programa PipeLine".
...
Nota: Ao configurar o programa, o usuário do QAT pode selecionar diversas áreas técnicas na lista suspensa de áreas técnicas.
