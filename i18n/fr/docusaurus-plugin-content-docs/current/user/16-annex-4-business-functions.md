...
* L'utilisateur A dispose uniquement d'un accès « Liste », il ne peut donc afficher que les données existantes (« lecture seule »)
...
* L'utilisateur C dispose d'un accès « Liste » + « Ajouter », ce qui lui permet d'ajouter de nouvelles entités et d'afficher la liste des entités.
