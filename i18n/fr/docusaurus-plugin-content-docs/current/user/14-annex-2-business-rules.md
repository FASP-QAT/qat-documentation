...
## B. Module de planification des approvisionnements : Planification des approvisionnements

1. **Solde d'ouverture :**
   Le solde d'ouverture d'un mois est le même que celui de fin du mois dernier et s'il s'agit du premier mois, le solde d'ouverture est de 0.
...
2. **Stock expiré :**
   Le stock expiré est calculé comme la somme du solde de clôture de tous les lots du mois précédent disponibles et dont la date d'expiration est la même que celle du mois en cours.
   
   **Formule :**
   *Stock expiré = Somme du solde final des lots qui expirent ce mois-ci*
...
4. **Stock final et ajustement :**
   ...
   un. Si la région a déclaré à la fois le stock réel et l'ajustement, nous prenons uniquement en compte le décompte réel lors de la somme et l'ajustement est ignoré pour cette région même si quelqu'un a saisi les données pour l'ajustement.
   b. Si la région a uniquement déclaré l'ajustement, cet ajustement est pris en compte lors de la somme.
...
5. **Consommation :**
   ...
   * Si toutes les régions n'ont pas déclaré la consommation réelle, QAT prend le maximum de la somme de la consommation prévue et de la somme de la consommation réelle.
...
6. **Inventaire projeté :**
   L'inventaire projeté est calculé comme la somme du solde d'ouverture, du total des expéditions, du total des ajustements et de la déduction de la consommation finale totale et du stock expiré.
   
   **Formule :**
   *Stock projeté = Solde d'ouverture + Total des expéditions +/- Total des ajustements - Consommation finale - Stock périmé*

7. **Calculer l'ajustement automatique :**
   Les ajustements automatiques sont calculés en fonction de trois conditions :
   une. Si toutes les régions ont déclaré l'inventaire réel et que l'inventaire projeté n'est pas égal à l'inventaire final
   b. Si certaines régions ont déclaré que l'inventaire réel et l'inventaire final sont supérieurs à la somme de l'inventaire projeté et de l'ajustement total.
   c. Si l'une des régions a déclaré l'inventaire réel et l'inventaire projeté est inférieur à 0
   
   Et sur la base des conditions ci-dessus, si l'une des conditions est remplie, l'ajustement automatique est calculé comme la différence entre l'inventaire final et l'inventaire projeté, sinon il est considéré comme égal à 0.
...
9. **MOS minimum :**
   ...
   Les règles pour les décimales sont les suivantes :
   * Afficher 0 décimale si plus de 3 chiffres (&gt;100, par exemple 10 000)
   * Afficher 1 décimale si 2 chiffres (par exemple 99,8)
   * Afficher 2 décimales si 1 chiffre (par exemple 1,63)
   * Afficher 3 décimales si inférieur à 0 (par exemple 0,124)
...
13. **Quantité maximale :**
    ...
    **Formule :**
    *Quantité maximale = Quantité minimale + (Fréquence de réapprovisionnement * AMC)*

14. **Solde final :**
    Le solde final est calculé en fonction de certaines conditions :
    une. Si toutes les régions ont déclaré l'inventaire réel, alors le solde final est égal au stock final.
      
      **Formule :**
      *Solde final = Stock final*
    b. Si certaines régions ont déclaré l'inventaire réel et si le stock final est supérieur à la somme du stock projeté et de l'ajustement automatique, alors le solde de clôture est égal au stock final.
      
      **Formule :**
      *Solde final = Stock final*
    c. Sinon, le solde final est calculé comme la somme de l’inventaire projeté et de l’ajustement automatique.
      
      **Formule :**
      *Solde final = Stock projeté + Ajustement automatique*
...
16. **MOS :**
    Le MOS est calculé comme le rapport du solde final à celui de l’AMC.
    
    **Formule :**
    *MOS = Solde final / AMC*

17. **Quand suggérer des expéditions (Planifier par MOS) :**
    QAT suggère l'expédition pour un mois lorsque le MOS du mois et des deux mois suivants est inférieur à celui du Min MOS et si le mois est supérieur au mois en cours.
    
    **Formule :**
    *Quand suggérer des expéditions = Si MOS pour le mois et les deux mois suivants &lt; Min MOS*
...
18. **Quantité d'expédition suggérée (Plan par MOS) :**
    ...
    * Si le MOS du mois en cours et des deux mois suivants est inférieur au Min MOS :
      *Quantité de commande suggérée = (Stock maximum - Solde final + Demande non satisfaite)*
    * Si le MOS est « 0 » et l'AMC est supérieur à &gt; 0 et que les deux mois suivants sont inférieurs au Min MOS :
      *Quantité de commande suggérée = (Stock maximum - Solde final + Demande non satisfaite)*
    * Si le MOS est « 0 » et l'AMC est supérieur à &gt; 0 et que l'un des deux mois suivants est supérieur au Min MOS :
      *Quantité de commande suggérée = (Stock minimum - Solde final + Demande non satisfaite)*
...
19. **Quand suggérer des expéditions (planifier par quantité) :**
    ...
    **Formule :**
    *Quand suggérer des expéditions = Si solde de fin pour le mois + X (délai de distribution) et les deux mois suivants &lt; Qté minimum ou*
    *Quand suggérer des expéditions = Si le solde final du mois = 0 et que l'AMC n'est pas égal à 0*
...
20. **Quantité d'expédition suggérée (planifier par quantité) :**
    ...
    **Formule :**
    * Si le solde de clôture du mois en cours + X mois (délai de distribution) et des deux mois suivants est inférieur à la quantité minimale :
      *Quantité de commande suggérée = (Quantité maximale pour le mois en cours + X mois (délai de distribution) - Solde final + Demande non satisfaite)*
    * Si le solde de clôture est « 0 » et que l'AMC est supérieur à &gt; 0 pour le mois en cours + X mois (délai de distribution) et que les deux mois suivants sont inférieurs à la quantité minimale :
      *Quantité de commande suggérée = (Quantité maximale pour le mois en cours + X mois (délai de distribution) - Solde final + Demande non satisfaite)*
    * Si la fin est « 0 » et que l'AMC est supérieur à &gt; 0 pour le mois en cours + X mois (délai de distribution) et que l'un des deux mois suivants est supérieur au Min MOS :
      *Quantité de commande suggérée = (Quantité minimale - Solde final + Demande non satisfaite)*
...
21. **Calculs par lots :**
    ...
    une. **Calculs FEFO :**
      ...
      je. Si la somme du réglage total et du réglage automatique est inférieure à 0, alors le FEFO non alloué est calculé comme la différence entre la consommation finale et la somme du réglage manuel et du réglage automatique.
        
        **Formule :**
        *FEFO non alloué = Consommation finale - (Ajustement manuel + Ajustement automatique)*
      ii. Si la somme de l’ajustement et de l’ajustement national est supérieure à 0 alors le FEFO non alloué est le même que celui de la consommation finale.
        
        **Formule :**
        *FEFO non alloué = Consommation finale*
      
      Nous trions tous les lots en fonction de la date d'expiration par ordre croissant, c'est-à-dire que nous trions les lots dans l'ordre qui expire en premier.
      
      Nous parcourons ensuite tous les lots pour effectuer les calculs FEFO :
      
      Nous calculons le solde d'ouverture d'un lot qui est le même que celui du solde de fin du même lot du mois précédent. Et si le lot est créé alors le solde d'ouverture est considéré comme 0.
      
      **Formule :**
      *Solde d'ouverture du lot nouvellement créé = 0*
      *Solde d'ouverture des lots restants = Solde de fin*
      ...
      **Formule :**
      *Stock expiré (si la date d'expiration est la même que celle de la date actuelle) = Solde de clôture*
      ...
      *Solde de fin temporaire = Solde d'ouverture + Quantité d'expédition + Quantité d'ajustement - Quantité de consommation - Stock expiré*
      ...
      je. Si le solde de fin temporaire est supérieur ou égal au FEFO non alloué, alors le solde de fin est égal à la différence entre le solde de fin temporaire et le FEFO non alloué. Dans ce cas également, nous définissons le FEFO non alloué à 0 puisque nous avons alloué toute la quantité au lot.
        
        **Formule :**
        *Solde de fin = Solde de fin temporaire - FEFO non alloué*
        *FEFO non alloué = 0*
      ii. Si le solde de fin temporaire est inférieur au FEFO non alloué, alors le solde de fin est égal à 0 puisque nous consommerons entièrement ce lot. De plus, nous déduisons ensuite le temp CB du FEFO non alloué puisque nous avons consommé autant de quantité.
...
b. **Calculs LEFO :**
      ...
      je. Si la somme du réglage manuel et du réglage automatique est supérieure à 0, alors le LEFO non alloué est calculé comme la différence entre 0 et la somme du réglage manuel et du réglage automatique.
        
        **Formule :**
        *LEFO non alloué = 0 - (Ajustement manuel + Ajustement automatique)*
      ii. Si la somme du réglage manuel et du réglage automatique est inférieure à 0, alors le LEFO non alloué est 0.
        
        **Formule :**
        *LEFO non alloué = 0*
      iii. Si LEFO non alloué n'est pas égal à 0 alors on continue sinon on interrompt.
      
      Nous trions tous les lots en fonction de la date d'expiration par ordre décroissant, c'est-à-dire que nous trions les lots dans l'ordre qui expire en premier.
      
      Nous parcourons ensuite tous les lots pour effectuer les calculs LEFO :
      ...
      **Formule :**
      *Solde de fin temporaire = Solde de fin après calculs FEFO*
      
      Nous calculons ensuite le montant du solde final qui est calculé en fonction de deux conditions différentes :
      je. Si le solde de fin temporaire est supérieur ou égal au LEFO non alloué, le solde de fin est égal à la différence entre le solde de fin temporaire et le LEFO non alloué. Dans ce cas également, nous définissons le LEFO non alloué à 0 puisque nous avons alloué toute la quantité au lot.
        
        **Formule :**
        *Solde de fin = Solde de fin temporaire - LEFO non alloué*
        *LEFO non alloué = 0*
      ii. Si le solde final temporaire est inférieur au LEFO non alloué, alors le solde final est égal à 0 puisque nous consommerons entièrement ce lot. De plus, nous déduisons ensuite la température CB du LEFO non alloué puisque nous avons consommé autant de quantité.
...
## C. Module de planification des approvisionnements : liste des problèmes QAT

###C1. Qualité des données

1. **Données de consommation réelles récentes manquantes (au cours des derniers &lt;%X %&gt; mois) :**
   une. **Suggestion :** *Veuillez indiquer la consommation réelle dans la région &lt;%REGION %&gt; pour le mois de &lt;%DT %&gt;.*
   b. **Logique :** QPL vérifiera la consommation réelle des trois derniers mois et du mois en cours. Si la consommation réelle des trois derniers mois et du mois en cours est manquante, le problème est signalé.

2. **Entrée d'inventaire récente manquante (au cours des &lt;%X%&gt; derniers mois) :**
   une. **Suggestion :** *Veuillez fournir l'inventaire des stocks dans la région &lt;%REGION %&gt; pour le mois de &lt;%DT%&gt;. *
   b. **Logique :** QPL vérifiera le stock des trois derniers mois et du mois en cours. Si le stock des trois derniers mois et du mois en cours est manquant, le problème est signalé.

3. **Inputs de consommation réelle récents manquants pendant des mois dans les écarts :**
   une. **Suggestion :** *Veuillez indiquer la consommation réelle dans la région &lt;%REGION %&gt; pour le mois de &lt;%DT %&gt;.*
   b. **Logique :**
     je. QPL vérifiera les entrées de consommation réelles pour le mois en cours (considéré comme t) et pour le mois qui sera le mois en cours moins deux mois (considéré comme t-2), c'est-à-dire t et t-2 si les deux mois ont une consommation réelle et le mois en cours moins un mois (considéré comme t-1), c'est-à-dire que t-1 n'a pas de consommation réelle. Dans ce cas, le problème est signalé.
     ii. QPL vérifiera les entrées de consommation réelles pour le mois en cours moins un mois (considéré comme t-1) et pour le mois qui sera le mois en cours moins trois mois (considéré comme t-3), c'est-à-dire t-1 et t-3 si les deux mois ont une consommation réelle et le mois en cours moins deux mois (considéré comme t-2), c'est-à-dire que t-2 n'a pas de consommation réelle. Dans ce cas, le problème est signalé.

###C2. Calendrier d'approvisionnement

1. **Les envois ont reçu des dates de réception antérieures à plus de &lt;%X%&gt; jours :**
   une. **Suggestion :** *Veuillez vérifier que l'envoi &lt;%SHIPMENT_ID %&gt; a été reçu et mettre à jour la date de réception ou l'état de l'envoi.*
   b. **Logique :** QPL vérifiera la date de réception de toutes les expéditions et si la date de réception est dépassée et a également dépassé la période tampon, le problème est signalé. Par exemple, le délai tampon pour ce problème est de 14 jours. Ainsi, si la date de réception de l'envoi plus les jours tampon sont dépassés, le problème est signalé pour chaque envoi.

2. **Les envois n'ont pas été soumis depuis plus de &lt;%X%&gt; jours depuis la date cible :**
   une. **Suggestion :** *En fonction des délais de livraison, l'expédition &lt;%SHIPMENT_ID%&gt; aurait dû être soumise maintenant. Veuillez revérifier pour vous assurer que le statut ou la date de réception est exact.*
   b. **Logique :** QPL vérifiera la date de soumission pour toutes les expéditions et si la date de soumission est dépassée et a également dépassé la période tampon, le problème est signalé. Par exemple, le temps tampon pour ce problème est de 3 jours. Par conséquent, si la date de soumission de l'envoi ainsi que les jours tampon sont déjà passés, le problème est signalé pour chaque envoi.

###C3. Planification des approvisionnements

1. **Aucune consommation prévue &lt;%X %&gt; mois dans le futur :**
   une. **Suggestion :** *Veuillez fournir la consommation prévue dans la région &lt;%REGION %&gt; pour le mois de &lt;%DT %&gt;.*
   b. **Logique :** QPL vérifiera la consommation prévue pour les 18 prochains mois. Si la consommation prévue pour l’un des mois manque, le problème est signalé. Le problème passe à l’état de conformité si et seulement si tous les 18 mois ont prévu une consommation.

2. **La prévision dynamique n'est pas utilisée pour les groupes de produits ARV :**
   une. **Suggestion :** *Veuillez vérifier les données de consommation dans la région &lt;%REGION%&gt; pendant la période &lt;%DT%&gt;, car nous encourageons les prévisions avec des valeurs mensuelles dynamiques, montrant que des facteurs tels que l'échelle du programme ou la saisonnalité ont été pris en compte.*
   b. **Logique :** QPL vérifiera si l'unité de planification du programme appartient au groupe de produits ARV, puis QPL vérifiera la consommation prévue pour les 18 mois à venir si quatre mois consécutifs au cours de la période de 18 mois à venir ont la même quantité de consommation prévue, alors le problème est signalé.

3. **La prévision dynamique n'est pas utilisée pour les groupes de produits PALUDISME :**
   une. **Suggestion :** *Veuillez vérifier les données de consommation dans la région &lt;%REGION %&gt; pendant la période &lt;%DT%&gt;, car nous encourageons les prévisions avec des valeurs mensuelles dynamiques, montrant que des facteurs tels que l'échelle du programme ou la saisonnalité ont été pris en compte.*
   b. **Logique :** QPL vérifiera si l'unité de planification du programme appartient au groupe de produits PALUDISME, puis QPL vérifiera la consommation prévue pour les 18 mois à venir. Si quatre mois consécutifs au cours de la période de 18 mois à venir ont la même quantité de consommation prévue, le problème est alors signalé.

4. **Les prévisions dynamiques ne sont pas utilisées pour les groupes de produits VMMC :**
   une. **Suggestion :** *Veuillez vérifier les données de consommation dans la région &lt;%REGION%&gt; pendant la période &lt;%DT%&gt;, car nous encourageons les prévisions avec des valeurs mensuelles dynamiques, montrant que des facteurs tels que l'échelle du programme ou la saisonnalité ont été pris en compte.*
   b. **Logique :** QPL vérifiera si l'unité de planification du programme appartient au groupe de produits VMMC, puis QPL vérifiera la consommation prévue pour les 18 mois à venir. Si quatre mois consécutifs au cours de la période de 18 mois à venir ont la même quantité de consommation prévue, le problème est signalé.

5. **Le plan d'approvisionnement ne suit pas les paramètres minimum/maximum :**
   une. **Suggestion :** *Réévaluer le plan d'approvisionnement pour répondre aux paramètres Min/Max. En cas de surstock, envisagez de retarder/annuler les expéditions, de vérifier les expirations futures ou de faire don de stock. En cas de sous-stockage, envisagez de planifier et de commander, d’accélérer ou d’augmenter la quantité d’expédition.*
   b. **Logique :** QPL vérifiera si chaque unité de planification est en surstock ou en sous-stock à tout moment au cours des 18 prochains mois en fonction des paramètres minimum et de réapprovisionnement définis au niveau de chaque programme. QAT comptera ensuite le nombre de mois pendant lesquels l'unité de planification est en surstock et en sous-stockage et le subdivisera en deux fenêtres temporelles différentes : les mois compris dans le délai d'exécution (1 à 6 mois) et ceux compris dans la fenêtre de planification (7 à 18 mois). Le QPL indiquera également le nombre d’expéditions reçues au cours des 1 à 6 prochains mois et des 7 à 18 prochains mois.

6. **Rupures de stock dans les 1 à 6 prochains mois/Rupures de stock dans les 7 à 18 prochains mois :**
   une. **Suggestion :** *Envisagez de planifier et de commander, d'accélérer ou d'augmenter la quantité d'expédition. Communiquer les ruptures de stock potentielles avec les parties prenantes concernées.*
...
###C4. Autre (problèmes manuels)

1. Il s'agit de questions ou de clarifications spécifiques saisies manuellement par le réviseur du plan d'approvisionnement qui ne sont pas automatiquement signalées par QAT. La description du problème, la suggestion et la criticité sont déterminées par l'examinateur du plan d'approvisionnement. QAT n'effectue aucune logique spécifique pour ce problème car il doit être ajouté, mis à jour et résolu manuellement par le réviseur du plan d'approvisionnement.

## D. Module de planification des approvisionnements : autres rapports QAT
...
**Formule :**
*WAPE = ((Abs(consommation réelle mois 1 - consommation prévue mois 1) + Abs(consommation réelle mois 2 - consommation prévue mois 2) + Abs(consommation réelle mois 3 - consommation prévue mois 3) + Abs(consommation réelle mois 4 - consommation prévue mois 4) + Abs(consommation réelle mois 5 - consommation prévue mois 5) + Abs(consommation réelle mois 6 - consommation prévue mois 6)) / (Somme de toute la consommation réelle au cours des 6 derniers mois))*
*Remarque : Fenêtre de temps considérée comme 6 mois*

### 2. Frais d'expédition :
Il s'agit du coût total requis pour recevoir la commande à l'entrepôt.
...
**Formule :**
*Coût de l'inventaire = solde final * prix catalogue*
...
#### Rotations de stock planifiées (IT)

**Au niveau PU :**
Le calcul se fait en déterminant les mois de stock possibles pour chaque mois sur 12 mois si les règles d'inventaire ont parfaitement fonctionné en tenant compte du MIN et de l'intervalle de réapprovisionnement de l'unité de planification.

Consommation = 1 MOS/mois ; Consommation totale sur période = 12 MOS. Le MOS moyen est calculé sur la base du MIN et de l'intervalle de réapprovisionnement.
...
#### Rotations réelles des stocks (IT)
...
4. Si la condition ci-dessus (20 %) est trouvée plusieurs fois dans l'ensemble de données, QAT doit utiliser l'inv la PLUS FAIBLE. tourne le mode de l'ensemble. *(Exemple : le programme contient 100 PU ; 60 PU ont des tours différents ; 20 PU ont des tours inv = 1,2, 20 autres PU ont des tours inv. = 2,5 ; QAT affichera 1,2 comme tours inv. pour ce programme)*
...
4. Règle : si la condition ci-dessus est trouvée plusieurs fois dans l'ensemble de données, QAT doit utiliser l'inv la PLUS FAIBLE. mode tours du groupe * (par exemple, le pays contient 1 000 PU ; 600 PU ont des tours différents ; 200 PU ont des tours inv = 1,6, 200 autres PU ont des tours inv. = 2,2 ; QAT affichera 1,6 comme tours inv. pour ce pays/catégorie PU)*.
...
**Formule :**
*Délai total de l'agent d'approvisionnement (par avion) = Délai prévu à soumis en mois + Soumis au délai approuvé en mois + Approuvé à expédié Délai en mois + Expédié à arrivé Délai en mois (par avion) + Arrivé à reçu Délai en mois*
*Délai total de l'agent d'approvisionnement (par mer) = Délai prévu à soumis en mois + Soumis au délai approuvé en mois + Approuvé à expédié Délai en mois + Expédié à arrivé Délai en mois (par mer) + Arrivé à reçu Délai en mois*
...
## E. Module de prévision : Prévisions d'arbres
...
### 2. Nœud numérique :
La valeur du nœud numérique pour un mois est définie au niveau du nœud.

**Formule :**
*Valeur du nœud pour un mois = Valeur définie au nœud*

Il existe trois types de modélisation/transfert pouvant être appliqués :
* **a. Numéro linéaire :**
  ...
  * **Formule de modélisation :** *Valeur pour un nœud pour un mois = Valeur définie au nœud +/- Quantité mensuelle fixe*
  * **Formule de transfert :**
    *Valeur du transfert depuis le nœud pour un mois = Valeur définie au niveau du nœud - quantité mensuelle fixe*
    *Valeur du transfert au nœud pour un mois = Valeur définie au nœud + quantité mensuelle fixe*
* **b. Pourcentage linéaire :**
  ...
  * **Formule de modélisation :** *Valeur d'un nœud pour un mois = Valeur définie au nœud +/- Pourcentage de la valeur du mois de départ*
  * **Formule de transfert :**
...
* **c. Pourcentage exponentiel :**
  ...
  Si un transfert de pourcentage exponentiel est appliqué, un pourcentage de la valeur du mois précédent est diminué à partir de ce nœud et le même montant est augmenté dans le nœud transféré pour chaque mois.
  * **Formule de modélisation :** *Valeur d'un nœud pour un mois = Valeur définie au nœud +/- Pourcentage de la valeur du mois précédent*
  * **Formule de transfert :**
    *Valeur du transfert depuis le nœud pour un mois = Valeur définie au niveau du nœud - Pourcentage de la valeur du mois précédent*
    *Valeur du transfert vers le nœud pour un mois = Valeur définie au nœud + Pourcentage de la valeur du mois précédent*

### 3. Nœud de pourcentage :
...
Il n’existe qu’un seul type de modélisation/transfert applicable :
* **a. Linéaire (point de %) :**
  Si une modélisation linéaire (points de %) est appliquée, la valeur est calculée comme la somme du pourcentage du nœud et de la valeur de pourcentage linéaire (points de %) du nœud parent.
  ...
  * **Formule de modélisation :** *Valeur d'un nœud pour un mois = (Valeur définie au nœud +/- Valeur en % linéaire) * Valeur du nœud du parent / 100*
  * **Formule de transfert :**
...
### 4. Nœud de l'unité de prévision :
...
### 5. Nœud de l'unité de planification :
...
## F. Module de prévision : Prévision de consommation
...
* Où les valeurs de données connues sont *(x₀ , y₀)* et *(x₁ , y₁)*
...
La consommation ajustée est calculée en fonction de la consommation réelle, du taux de déclaration, du taux de rupture de stock (jours) et du nombre de jours dans le mois en utilisant la formule ci-dessous.
...
* QAT permet aux utilisateurs d'importer les données réelles de leurs plans d'approvisionnement dans une prévision de consommation.
  ...
  * Après avoir sélectionné suivant, un % du plan d'approvisionnement doit maintenant être saisi.
    * *Une info-bulle s'affiche dans la colonne supérieure indiquant « Pourcentage de la quantité de consommation réelle mensuelle du programme du plan d'approvisionnement qui sera importée dans le programme de prévision.*
    * *Ceci permet d'utiliser un pourcentage spécifique si les 100 % ne sont pas nécessaires.*

## G. Module de prévision : rapports et résultats

###G1. Comparez et sélectionnez

1. **Comparer aux prévisions de consommation :**
   Comparer aux prévisions de consommation s'affiche uniquement lorsqu'il existe à la fois des prévisions d'arborescence et de consommation. Il affiche les valeurs comme :
...
3. **Excédent/écart en matière d'approvisionnement**
...
4. **Approvisionnements nécessaires**
   Les achats nécessaires sont calculés en multipliant l’excédent/l’écart des achats et le prix unitaire.

   **Formule :**
   *Approvisionnements nécessaires = Excédent d'approvisionnement * Prix unitaire*
...
## H. Module de planification et de prévision des approvisionnements : téléchargement de versions
...
une. Serveur v8 (il s'agit de la version du serveur que l'utilisateur a téléchargée avant d'apporter des modifications)
b. Local v8 (c'est la version que l'utilisateur a l'intention de télécharger)
c. Serveur v10 (c'est la dernière version du serveur)
...
* <span style={{color: '#00b050', fontWeight: 'bold'}}>Nouvellement ajouté sur la version locale (Local v8)</span> &larr; Condition 1
* <span style={{color: '#0070c0', fontWeight: 'bold'}}>Nouvellement ajouté sur la version serveur (Serveur v10)</span> &larr; Condition 2
* **Inchangé depuis la version précédente du serveur (Serveur v8)** &larr; Condition 3a
* <span style={{color: '#00b050', fontWeight: 'bold'}}>Mise à jour sur la version locale (Local v8) sans conflits</span> &larr; Condition 3b
* <span style={{color: '#0070c0', fontWeight: 'bold'}}>Mise à jour sur la version serveur (Serveur v10) sans conflits</span> &larr; Condition 3c
...
### a. Condition 1 : données nouvellement ajoutées dans la version locale

Tout d'abord, QAT vérifie chaque point de données dans la version locale (Local v8) pour confirmer s'il s'agit de données nouvellement ajoutées. Si tel est le cas, les données s'affichent en <span style={{color: '#00b050', fontWeight: 'bold'}}>vert</span> sur l'écran de téléchargement.

* **a.** Serveur v8 - L'enregistrement n'existe pas
* <span style={{color: '#00b050', fontWeight: 'bold'}}>**b.** Local v8 - L'enregistrement existe</span>
* **c.** Serveur v10 - L'enregistrement n'existe pas

Voici quelques exemples de cette condition :
1. **Consommation (Planification des approvisionnements) :** Par exemple, il existe un enregistrement de consommation réelle dans la version locale v8 pour mai 2023, unité de planification A, unité de reporting alternative B, région X. QAT vérifie le serveur v10 et le serveur v8 pour voir s'il existe un enregistrement de consommation réelle en mai 2023, unité de planification A, unité de reporting alternative B, région X. Sinon, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
2. **Inventaire (Planification des approvisionnements) :** Par exemple, il existe un enregistrement d'inventaire dans la version locale v8 pour mai 2023, unité de planification A, unité de reporting alternative B, région X, source de données Y. QAT vérifie le serveur v10 et le serveur v8 pour voir s'il existe un enregistrement d'inventaire en mai 2023, unité de planification A, unité de reporting alternative B, région X, source de données Y. Sinon, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
3. **Ajustements, expéditions, QPL (Supply Planning) :** Étant donné que les ID ne sont attribués qu'après le téléchargement du plan d'approvisionnement, QAT vérifie les ID de chaque enregistrement dans la version locale 8 pour voir s'ils sont « 0 ». Les enregistrements portant l'ID « 0 » ont été nouvellement créés dans cette version locale, donc pour ces enregistrements, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span> QAT ne vérifie pas les versions du serveur.
4. **Arbres (prévisions) :** Étant donné que les ID ne sont attribués qu'après le téléchargement de la prévision, QAT vérifie les ID de chaque arbre dans la version locale 8 pour voir s'ils sont « 0 ». Les arbres avec l'ID « 0 » ont été nouvellement créés dans cette version locale, donc pour ceux-ci, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span> QAT ne vérifie pas les versions du serveur.
5. **Paramètres de l'unité de planification (prévision) :** QAT vérifie si cet ID d'unité de planification est présent dans le serveur v8 et le serveur v10. Si l'ID de l'unité de planification n'existe pas dans l'un ou l'autre, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
6. **Données de consommation et extrapolation (prévision) :** QAT vérifie si les données de cette unité de planification et de cette région sont présentes dans le serveur v10 en fonction de la présence de la date modifiée. Si les données de l'unité de planification et de la région ne sont pas présentes dans le serveur v10, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
7. **Prévision sélectionnée (Prévision) :** QAT vérifie si une prévision sélectionnée existe pour cette unité de planification et cette région dans Server v10. Si aucune sélection n'est présente dans le serveur v10, <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>

###b. Condition 2 : données nouvellement ajoutées dans la version serveur

Si la condition 1 échoue, QAT vérifie la dernière version du serveur (Serveur v10) pour voir si ces données ont été récemment ajoutées en les comparant avec la version locale (Local v8). QAT ne vérifie pas la version précédente du serveur (serveur v8). Si les données ne sont pas présentes sur la version locale, cela signifie qu'elles ont été récemment ajoutées sur la dernière version du serveur et qu'il n'y a pas de conflit avec la version locale. Ces données seront ajoutées à la version nouvellement téléchargée (Serveur v11) et les données sont affichées en <span style={{color: '#0070c0', fontWeight: 'bold'}}>bleu</span> dans l'écran de téléchargement.

* **a.** Serveur v8 - L'enregistrement n'existe pas
* **b.** Local v8 - L'enregistrement n'existe pas
* <span style={{color: '#0070c0', fontWeight: 'bold'}}>**c.** Serveur v10 - L'enregistrement existe</span>

Voici quelques exemples de cette condition :
1. **Inventaire de consommation, ajustement, expédition, QPL (planification des approvisionnements) :** Par exemple, il existe un enregistrement de consommation réelle avec l'ID unique #A dans le serveur v10. QAT vérifie si l'ID#A unique est également présent dans la version locale v8. Si l'ID unique n'est pas présent dans la version locale, le serveur <span style={{color: '#0070c0', fontWeight: 'bold'}}> v10 est le plus récent.</span>
2. **Arbres :** Par exemple, il existe un arbre avec l'ID unique #B sur le serveur v10. QAT vérifie si l'ID#B unique est également présent dans la version locale v8. Si l'ID unique n'est pas présent dans la version locale, le serveur <span style={{color: '#0070c0', fontWeight: 'bold'}}> v10 est le plus récent.</span>
3. **Paramètres de l'unité de planification (prévision) :** Par exemple, il existe une unité de planification avec l'ID d'unité de planification #C dans le serveur v10. QAT vérifie si l'unité de planification #C est également présente dans la version locale v8. Si l'ID de l'unité de planification n'est pas présent dans la version locale, le serveur <span style={{color: '#0070c0', fontWeight: 'bold'}}> v10 est le plus récent.</span>
4. **Données de consommation et extrapolation (prévision) :** Une fois que QAT a trouvé les données d'une unité de planification et d'un serveur de région v10, QAT examine la même unité de planification et la même région dans la version locale v8 recherche une date modifiée. Si la date de modification n'existe pas dans la v8 locale, le <span style={{color: '#0070c0', fontWeight: 'bold'}}>serveur v10 est la dernière.</span>
5. **Prévision sélectionnée (prévision) :** Une fois que QAT a trouvé des données pour des prévisions sélectionnées pour une unité de planification et une région particulières dans le serveur v10, QAT recherche la même unité de planification et la même région dans la version locale v8 recherche une prévision sélectionnée. Si la prévision sélectionnée n'existe pas dans la version locale v8, le <span style={{color: '#0070c0', fontWeight: 'bold'}}>serveur v10 est la dernière.</span>

###c. Condition 3a : Le serveur local et le dernier serveur sont les mêmes

Si les conditions ci-dessus échouent, QAT vérifie chaque point de données dans la version locale (Local v8) pour voir s'ils sont identiques à la dernière version du serveur (v10). Si c'est le cas, les données sont affichées sans surbrillance dans l'écran de téléchargement.

* **a.** serveur v8 – L'enregistrement est X
...
2. **Consommation (Prévision) et Arbres :** QAT compare la date de dernière modification de chaque enregistrement dans la version locale (v8) avec la date de dernière modification de la dernière version du serveur (v10). S'ils sont identiques, aucune modification n'a été apportée depuis le serveur v8.

### d. Condition 3b : Modifications uniquement dans la version locale

Si les conditions ci-dessus échouent, QAT compare pour voir si la version précédente du serveur (Serveur v8) et la dernière version du serveur (serveur v10) sont identiques. Si tel est le cas, QAT déduit que la version locale (v8 locale) a été modifiée et les données sont affichées en <span style={{color: '#00b050', fontWeight: 'bold'}}>vert</span> dans l'écran de téléchargement.

* **a.** serveur v8 – la source de données est les rapports SIGL
* <span style={{color: '#00b050', fontWeight: 'bold'}}>**b.** v8 local – la source de données est Warehouse Reports</span>
* **c.** dernier serveur v10 - la source de données est les rapports LMIS

Voici quelques exemples de cette condition :
1. **Consommation, inventaire, ajustement, expéditions, QPL, paramètres de version, paramètres d'unité de planification et prévisions sélectionnées :** Par exemple, un enregistrement de consommation réelle pour le 22 juin a « Rapports d'entrepôt » comme source de données dans la version locale 8. Si le champ de source de données pour les versions précédente (v8) et la dernière (v10) du serveur est tous deux « Rapports LMIS », cela indique que les données ont été modifiées dans la version locale, donc <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
2. **Consommation (Prévision) et Arbres :** Si les dates de dernière modification des versions précédente (v8) et dernière (v10) du serveur sont les mêmes, cela indique que les données ont été modifiées en version locale, donc <span style={{color: '#00b050', fontWeight: 'bold'}}>local v8 est la dernière.</span>
   * **a.** Serveur v8 - la date de dernière modification est le 15 septembre 2023, 12:00:00
   * <span style={{color: '#00b050', fontWeight: 'bold'}}>**b.** Local v8 - la dernière date de modification est le 20 septembre 2023, 15:15:00</span>
   * **c.** Serveur v10 - la date de dernière modification est le 15 septembre 2023, 12:00:00

### e. Condition 3c : Modifications uniquement dans la dernière version du serveur

Si les conditions ci-dessus échouent, QAT compare pour voir si la version précédente du serveur (Serveur v8) et la version locale (local v8) sont identiques. Si tel est le cas, QAT déduit que la dernière version du serveur (serveur v10) a été modifiée et les données sont affichées en <span style={{color: '#0070c0', fontWeight: 'bold'}}>bleu</span> dans l'écran de téléchargement.

* **a.** Serveur v8 - la source de données est les rapports SIGL
* **b.** Local v8 - la source de données est les rapports SIGL
* <span style={{color: '#0070c0', fontWeight: 'bold'}}>**c.** Serveur v10 - la source de données est Warehouse Reports</span>

Voici quelques exemples de cette condition :
1. **Consommation, inventaire, ajustement, expéditions, QPL, paramètres de version, paramètres d'unité de planification et prévisions sélectionnées :** Par exemple, un enregistrement de consommation réelle pour le 22 juin. Si la valeur de la source de données dans le serveur v8 et dans la v8 locale est la même, cela indique que les données ont été modifiées dans la dernière version du serveur (v10), donc <span style={{color: '#0070c0', fontWeight: 'bold'}}> le serveur v10 est la dernière.</span>
2. **Consommation (Prévision) et Arbres :** Si les dates de dernière modification de la version précédente du serveur (v8) et de la version locale (v8) sont les mêmes, cela indique que les données ont été modifiées dans la dernière version du serveur (v10), donc <span style={{color: '#0070c0', fontWeight: 'bold'}}>serveur v10 est la dernière.</span>
   * **a.** Serveur v8 - la date de dernière modification est le 15 septembre 2023, 12:00:00
   * **b.** Local v8 - la dernière date de modification est le 15 septembre 2023, 12:00:00
   * <span style={{color: '#0070c0', fontWeight: 'bold'}}>**c.** Serveur v10 - la dernière date de modification est le 20 septembre 2023, 15:30:00</span>

###f. Condition 3d : modifications dans la version locale et la dernière version du serveur (même paramètre)

Si les conditions ci-dessus échouent, cela signifie que les 3 versions ne correspondent pas et qu'il y a un conflit. Si tel est le cas, les données sont mises en surbrillance <span style={{backgroundColor: '#ffff00'}}>jaune</span> dans l'écran de téléchargement.

* **a.** serveur v8 - la source de données est les rapports LMIS
* <span style={{backgroundColor: '#ffff00'}}>**b.** v8 local - la source de données est les rapports SDP</span>
* <span style={{backgroundColor: '#ffff00'}}>**c.** dernier serveur v10 - la source de données est Warehouse Reports</span>

Voici quelques exemples de cette condition :
1. **Consommation, inventaire, ajustement, expéditions, QPL, paramètres de version, paramètres d'unité de planification et prévisions sélectionnées :** Pour un enregistrement de consommation réelle pour le mois du 22 juin, si la valeur de la source de données ne correspond à aucune des versions, cela indique que les données sont modifiées dans les versions locale (v8) et la dernière version du serveur (v10). QAT met donc en surbrillance la ligne dans <span style={{backgroundColor: '#ffff00'}}>jaune</span> pour indiquer un conflit que l'utilisateur doit résoudre.
2. **Consommation (prévision) et arbres :** Si la date de dernière modification ne correspond à aucune des versions, cela indique que les données sont modifiées à la fois dans les versions locale (v8) et la dernière version du serveur (v10). QAT met donc en surbrillance la ligne en <span style={{backgroundColor: '#ffff00'}}>jaune</span> pour indiquer un conflit que l'utilisateur doit résoudre.
   * **a.** Serveur v8 - la date de dernière modification est le 15 septembre 2023, 12:00:00
   * <span style={{backgroundColor: '#ffff00'}}>**b.** Local v8 - la dernière date de modification est le 16 septembre 2023, 13:00:00</span>
   * <span style={{backgroundColor: '#ffff00'}}>**c.** Serveur v10 - la dernière date de modification est le 20 septembre 2023, 15:30:00</span>
   * **a.** Serveur v8 - la date de dernière modification est le 28 septembre 2023, 12:00:00
   * <span style={{backgroundColor: '#ffff00'}}>**b.** Local v8 - la dernière date de modification est le 16 septembre 2023, 13:00:00</span>
   * <span style={{backgroundColor: '#ffff00'}}>**c.** dernier serveur v10 – l'arborescence est manquante</span>

###g. Condition 4 : Modifications de la version locale et de la dernière version du serveur (différents paramètres)

Après avoir vérifié toutes les conditions, QAT vérifie s'il existe des enregistrements contenant une combinaison de 3b et 3c, mais pour des champs différents. Si tel est le cas, les données sont mises en surbrillance <span style={{backgroundColor: '#ffff00'}}>jaune</span> dans l'écran de téléchargement.

1. **Consommation, Stock, Ajustement, Expéditions, QPL, paramètres de version, paramètres d'unité de planification et prévisions sélectionnées :** Pour un enregistrement de consommation réelle du mois du 22 juin, la source de données est mise à jour dans la version locale (Condition 3b) et la quantité est modifiée dans la version serveur (condition 3c). Dans ce cas, QAT ne sait pas quelle version utiliser, donc QAT met en surbrillance la ligne en <span style={{backgroundColor: '#ffff00'}}>jaune</span> pour indiquer un conflit que l'utilisateur doit résoudre.
