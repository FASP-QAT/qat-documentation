...
## B. Niveaux de hiérarchie dans QAT
...
Pour en savoir plus sur les fonctionnalités destinées aux administrateurs au niveau des applications et des domaines, veuillez consulter [Annexe 1 : Manuel de l'administrateur des applications et des domaines.](13-annex-1-application-realm-administrator-manual.md#annex-1-application-realm-administrator-manual)
...
## C. Rôles et autorisations des utilisateurs
...
Veuillez vous référer à [Annexe 4 : Fonctions commerciales](16-annex-4-business-functions.md#annex-4-business-functions) pour un tableau avec la liste complète des fonctions commerciales QAT disponibles par rôle d'utilisateur.

## D. Comment se connecter à QAT et installer l'application Web progressive (PWA)
...
2. Une fois le compte créé, le nouvel utilisateur reçoit un e-mail pour « Réinitialiser le mot de passe ».
3. Le lien « Réinitialiser le mot de passe » redirigera l'utilisateur vers le site Web de QAT pour créer le nouveau mot de passe. Une fois terminé, cliquez sur « Soumettre ».
   un. Remarque : le lien « Réinitialiser le mot de passe » sera valable 24 heures et ne pourra être utilisé qu'une seule fois. Si nécessaire, l'utilisateur peut générer à nouveau le lien pour réinitialiser le mot de passe en cliquant sur « Mot de passe oublié » sur l'écran de connexion.
...
**Remarque :** Le mot de passe doit : comporter au moins six caractères, commencer par une lettre, ne pas être identique à votre nom d'utilisateur ou à votre dernier mot de passe, et doit inclure au moins un caractère spécial, un chiffre et une lettre majuscule.
...
1. Accédez à https://www.quantificationanalytics.org/
...
**Remarque** : Les utilisateurs peuvent utiliser simultanément les versions PWA et navigateur ; cependant, lorsque vous apportez des modifications dans l'un, une actualisation est nécessaire dans l'autre pour que les modifications soient prises en compte (voir [Quand et comment effectuer une actualisation d'application](#1-application-refresh)).

**Étapes pour installer PWA lorsque le bouton d'installation n'est pas disponible**
1. Dans votre navigateur Chrome, accédez à https://www.quantificationanalytics.org/
2. Cliquez sur les trois points verticaux dans le coin supérieur droit du navigateur
...
6. Cliquez sur l'icône "QAT" pour ouvrir l'application PWA de QAT.

![Install PWA alternate method](/img/media/image12.png)

_Installer une méthode alternative PWA_

## E. Comment se connecter à QAT pour les utilisateurs existants

**Connexion**
1. Pour vous connecter, saisissez l'URL https://www.quantificationanalytics.org/ dans la barre du navigateur et appuyez sur Entrée, ou ouvrez la PWA depuis votre bureau.
...
_Écran de connexion_
   ...
   une. Tapez le nom d'utilisateur et le mot de passe dans les champs obligatoires.
   b. Cliquez sur le bouton « Connexion ».
      1. Remarque : un utilisateur peut choisir de se connecter « en ligne » ou « hors ligne » en utilisant la case à cocher située au-dessus du bouton « Connexion ». Pour plus d'informations sur le travail hors ligne dans QAT, consultez la section F. [Travailler hors ligne](#working-offline).

2. Lors de la connexion en mode « en ligne » – QAT subira une synchronisation des données de base. Si vous voyez un message concernant une « version de serveur plus récente » – voir la figure 36 de la section 3.B pour plus d'informations.
...
4. Le nouveau mot de passe doit répondre aux critères suivants :
   <ul style={{ listStyleType: 'disc' }}>
     ...
     <li>Le mot de passe doit contenir au moins 1 chiffre</li>
     <li>Password must contain at least 1 uppercase letter</li>
     <li>Le mot de passe doit commencer par une lettre</li>
...
**Remarque :** *Un mot de passe utilisateur QAT sera valide jusqu'à un an à compter de la date de création/mise à jour.*

## F. Travailler hors ligne
...
| :---: | :---: |
| *Bouton hors ligne sur la page de profil* | *Bouton hors ligne sur la page de connexion* |
...
_Icône hors ligne ou en ligne_

Le tableau ci-dessous détaille certaines des fonctionnalités clés qui peuvent être réalisées dans QAT hors ligne ainsi que certaines des fonctionnalités qui doivent être réalisées en ligne :
...
| Afficher les rapports du programme et les résultats prévisionnels | Afficher les rapports mondiaux |
...
## Mises à jour de la version G. QAT
...
_Invite de publication d'une nouvelle version de QAT pour les utilisateurs_

En cas de changement majeur dans la structure de l'architecture backend de QAT, les utilisateurs peuvent être invités à télécharger leur dernière version sur le serveur (voir la section [Upload](06-managing-programs-and-versions.md#upload) pour plus d'informations) pour éviter de perdre des données une fois la nouvelle version de QAT publiée. L'équipe d'assistance QAT (support@quantificationanalytics.org) informera les utilisateurs bien à l'avance si un téléchargement de programme est nécessaire.

## H. Dépannage

Lorsqu'un utilisateur rencontre un problème système potentiel avec QAT, il doit tenter de résoudre le problème en essayant les solutions décrites dans cette section, dans l'ordre dans lequel elles sont décrites (c'est-à-dire, essayez d'abord H1, et si cela ne fonctionne pas, essayez H2, puis H3). Si aucune de ces étapes ne résout le problème, l'utilisateur doit soumettre un ticket au support technique pour signaler le bogue (voir la section « [Signaler un bug](#reporting-a-bug) »).

###H1. Quand et comment effectuer une actualisation de l'application

L'équipe de développement de QAT déploie des versions logicielles incrémentielles pour corriger les bogues et les modifications nécessaires à l'application. QAT invite les utilisateurs à le faire sur l'écran de connexion après chaque version (voir Figure 12). Pour effectuer une actualisation de l'application, les utilisateurs doivent appuyer simultanément sur "Ctrl" + "Shift" + "R" sur leur clavier.
...
- Dans un MDS complet, l'intégralité des données maître est chargée quelle que soit la dernière fois que QAT a été synchronisé, comme si c'était la première fois que vous chargez des données maître. Cela prendra donc plus de temps qu’un MDS normal.

![Click "Full Master Data Sync"](/img/media/image24.png)

_Synchronisation complète des données de base_

###H3. Quand et comment effacer les données du site
...
_Menu du service d'assistance QAT_

3. Choisissez Ajouter/Mettre à jour un utilisateur ou Ajouter/Mettre à jour les données principales. L’une ou l’autre option vous amènera à différents écrans pour plus d’informations.
4. Entrez les informations requises dans le formulaire et cliquez sur « Soumettre ».

![Add/Update User Screen](/img/media/image30.png)

_Ajouter/Mettre à jour l'écran utilisateur_

| ![Add Master Data Screen](/img/media/image31.png) | ![Add Procurement Agent Master Data Screen](/img/media/image32.png) |
| :---: | :---: |
| *Ajouter un écran de données de base* | *Ajouter un écran de données principales d'agent d'approvisionnement* |
...
_Modifier le ticket de demande_

4. Après avoir soumis un ticket, l'utilisateur se verra attribuer un numéro de code de ticket (ex : QAT-570). L'utilisateur recevra également une notification par e-mail et le nombre de « tickets ouverts » sur le ruban supérieur en rouge augmentera. Ce numéro de ticket correspond au numéro de ticket stocké dans l'outil de gestion du logiciel JIRA. Pour plus d'informations sur la gestion des tickets et sur l'outil de gestion du logiciel JIRA, veuillez consulter [Gestion des tickets](#managing-tickets).

### I3. Signaler un bug
Lorsqu'un utilisateur rencontre un problème système potentiel avec QAT, il doit tenter de le résoudre via une [actualisation de l'application] (#h1-quand-et-comment-effectuer-une-actualisation-d'application) et/ou [effacer les données du site] (#h3-quand-et-comment-effacer-les-données-du-site). Si aucune de ces étapes ne résout le problème, l'utilisateur doit soumettre un ticket au support technique pour signaler le bogue.
...
2. Saisissez un résumé du bug ainsi qu'une description du bug. Veuillez être descriptif et fournir autant de détails que possible (par exemple, sur quel programme/version/fonctionnalité vous travailliez, les étapes que vous avez suivies pour obtenir le bogue, etc.).
3. Téléchargez une capture d'écran du bug en cliquant sur le bouton « Parcourir » et en extrayant un fichier de capture d'écran enregistré de votre ordinateur.
   un. Il est également utile de fournir l'exportation du programme, si possible (voir [Exportations de programmes](06-managing-programs-and-versions.md#export) pour savoir comment exporter un programme local)
   b. **Remarque :** Si vous souhaitez télécharger plusieurs fichiers, placez les multiples captures d'écran dans un dossier sur votre ordinateur et compressez-le avant de naviguer dans QAT. Ensuite, téléchargez le dossier compressé.
4. Choisissez une priorité (la plus élevée, élevée, moyenne ou faible) pour le bug en fonction de l'urgence et de la manière dont il affecte le travail quotidien dans QAT.
   une. Les délais de réponse et de résolution des bogues seront basés sur le niveau de priorité
...
| :---: | :---: |
| *Numéro de code du ticket du service d'assistance* | *Nombre de billets sur le ruban supérieur* |
...
_Notification par e-mail pour le ticket QAT_

![Option to Navigate to JIRA](/img/media/image37.png)

_Option pour accéder à JIRA_

![JIRA Ticket Management](/img/media/image38.png)

_Gestion des tickets JIRA_

## J. Navigation et barre de menus
...
| :---: | :--- |
| ![Logout](/img/media/image46.png) | Déconnecte l'utilisateur de QAT. |
| ![Profile](/img/media/image47.png) | L'icône de profil est verte lorsque vous travaillez en ligne et rouge lorsque vous travaillez hors ligne. Lorsque l'utilisateur clique sur l'icône, il peut :<ul><li>Afficher le nom d'utilisateur et les rôles associés qui lui sont attribués</li><li>Modifier sa langue préférée</li><li>Modifier sa langue préférée mot de passe</li><li>Aller hors ligne/en ligne</li><li>Passer en mode thème sombre/clair</li><li>Afficher les décimales dans la planification des approvisionnements écrans</li></ul> |
| ![Home](/img/media/image42.png) | Renvoie l'utilisateur au tableau de bord/écran d'accueil. |
| ![Download Manual](/img/media/image48.png) | Télécharge le manuel d'utilisation de QAT sur l'ordinateur de l'utilisateur. Le fichier se trouve dans le dossier **Téléchargements** sur l'ordinateur de l'utilisateur. |
| ![Refresh](/img/media/image49.png) | Actualise la page actuelle sur laquelle un utilisateur travaille. |
| ![Upload Alert](/img/media/image50.png) | Informe l'utilisateur lorsqu'un programme stocké sur le serveur local comporte des modifications non téléchargées et doit télécharger ce programme sur le serveur. Si l'icône est bleue, aucun programme n'est non téléchargé. Si l'icône est rouge, certains programmes doivent être téléchargés sur le serveur. Le nombre dans la case rouge indique le nombre de programmes qui doivent être téléchargés. |
| ![Version Alert](/img/media/image51.png) | Informe l'utilisateur lorsqu'il existe une version plus récente sur le serveur pour l'un de ses programmes téléchargés localement. Si l'icône est bleue, l'utilisateur dispose de la version la plus récente de tous les programmes téléchargés. Si l'icône est rouge, certains programmes sont obsolètes par rapport au serveur. Le nombre dans la case rouge indique le nombre de programmes dont les versions sont obsolètes. |
| ![Helpdesk](/img/media/image52.png) | Ouvre le Helpdesk QAT où les utilisateurs peuvent créer des tickets pour ajouter/mettre à jour des données de base, proposer des demandes de modification et signaler un bug. Le nombre dans la case rouge indique le nombre de tickets ouverts dont dispose un utilisateur. Pour plus d'informations sur le service d'assistance, reportez-vous à QAT Helpdesk et Tickets. |
| ![ERP Notification](/img/media/image53.png) | Dirige l’utilisateur vers la page de notification d’expédition ERP. Cette page informera les utilisateurs s'il existe des mises à jour importantes qui doivent être examinées pour un envoi ERP lié. Pour plus d'informations sur ces notifications et celles qui seront signalées, reportez-vous à ERP Shipment Linking. Remarque : cette icône est uniquement disponible dans le module de planification des approvisionnements QAT. |
| ![Guidance](/img/media/image54.png) | Ouvre une page Web Afficher les conseils sur une introduction aux prévisions QAT. Ce document fournit un aperçu de haut niveau du flux du processus de prévision QAT. Remarque : Cette icône est uniquement disponible dans le module de prévision QAT. |
...
- **Module de planification des approvisionnements**
  ...
  - **Rapports** – Afficher les rapports. Modifier la liste des problèmes QAT.
- **Module de prévision**
  - **Master Data Sync** - En cliquant sur ce bouton, vous synchroniserez les données principales mises à jour avec votre version locale. Cette fonction est effectuée automatiquement lors de la connexion à QAT, mais ce bouton permet à l'utilisateur de le faire sans se déconnecter ni se reconnecter.
  - **Application Masters** (uniquement disponible pour les administrateurs d'applications et de domaines) – afficher/ajouter/modifier des données au niveau de l'application.
  - **Realm Level Masters** – afficher/ajouter/modifier les données au niveau du domaine.
  ...
  - **Prévisions basées sur la consommation** – Importer/ajouter/ajuster et extrapoler les données de consommation.
...
| Capture d'écran | Fonction |
| :---: | :--- |
| ![Hamburger icon](/img/media/image58.png) | Réduit complètement le menu de la barre latérale pour créer plus d'espace sur l'écran. En cliquant une fois sur l'icône, le menu disparaîtra. En cliquant à nouveau dessus, il réapparaîtra. |
| ![Left arrow icon](/img/media/image59.png) | Réduit le menu de la barre latérale sans le faire disparaître complètement. Crée un peu plus d'espace sur l'écran. |
| ![Right arrow icon](/img/media/image60.png) | Affiché lorsque le menu de la barre latérale est réduit. Cliquez pour développer et afficher le menu complet de la barre latérale. Également utilisé pour développer et afficher les éléments de sous-menu sous les éléments de menu. |
| ![Down arrow icon](/img/media/image61.png) | Affiché lorsque les éléments du sous-menu sont affichés. Cliquez pour réduire les éléments du sous-menu. |
| ![Module toggle buttons](/img/media/image62.png) | Boutons qui permettent aux utilisateurs de basculer entre le module de prévision et de planification des approvisionnements dans QAT, si leur accès utilisateur le permet. |
...
| Boutons | Actions |
| :---: | :--- |
| ![Submit button](/img/media/image63.png) | Soumet et apporte des modifications. |
| ![Cancel button](/img/media/image64.png) | Annule les modifications non soumises. |
| ![Reset button](/img/media/image65.png) | Rétablit les données non soumises au format précédent. |
| ![Plus icon button](/img/media/image66.png) | Ajoute un nouvel enregistrement à la base de données. |
| ![Update](/img/media/image67.png) | Met à jour les enregistrements. |
| ![Search](/img/media/image68.png) | Aide les utilisateurs à rechercher les informations souhaitées. Recherche tous les champs du tableau affiché. |
| ![Clear](/img/media/image69.png) | Permet à un utilisateur d'effacer les champs de recherche. |
| ![Red asterisk](/img/media/image70.png) | Indique les champs de saisie de données obligatoires. |
| ![Show Formulae](/img/media/image71.png) | Affiche des explications/définitions et des formules sur les termes couramment utilisés dans la fenêtre/le rapport spécifié. |
| ![Show Guidance](/img/media/image72.png) | Fournit des conseils de haut niveau sur la meilleure façon d’utiliser la page actuelle. |
| ![PDF, CSV, Word](/img/media/image73.png) | Exporte le rapport PDF, CSV ou Word (les options dépendent de l'écran). |

## K. Changer de langue
Pour garantir que QAT soit adaptable aux utilisateurs du monde entier, l'application affiche le texte dans quatre langues différentes : anglais, français, portugais et espagnol. Les utilisateurs sont encouragés à soumettre des tickets s’ils souhaitent voir une traduction d’étiquette particulière améliorée. (*Remarque : les traductions des étiquettes d'application ne peuvent être modifiées que par les **administrateurs d'application**. Les instructions sur la façon de procéder peuvent être trouvées dans la [section de traduction](13-annex-1-application-realm-administrator-manual.md#a-database-translation).*)

Il existe deux manières de modifier la langue affichée pour l'utilisateur :
1. Écran de connexion :
Dans le coin supérieur droit de l'écran de connexion, il y a un menu déroulant dans lequel les utilisateurs peuvent sélectionner leur
langue préférée avant la connexion. Cela modifiera non seulement le texte une fois connecté à QAT, mais
cela modifiera également le texte affiché sur l’écran de connexion.

![Login Screen Language Change](/img/media/image75.png)

_Changement de langue de l'écran de connexion_
...
_Dans le changement de langue de l'outil_

## L. Tableau de bord QAT
...
_Tableau de bord de planification des approvisionnements_

1. **Ticker** – Cette section affiche les informations clés à travers trois vignettes à défilement automatique.
Pour parcourir manuellement les vignettes, cliquez sur les points en bas des vignettes. Chaque vignette renvoie aux écrans pertinents. Cliquez sur le nom de la vignette pour être dirigé vers cet écran (si vous n'y avez pas accès, un message approprié s'affichera). Par exemple, en cliquant sur la vignette « Liaison ERP », vous accéderez à l’écran Liaison ERP.
    - **Realm :** Fournit des mesures globales, telles que le nombre de pays, d'utilisateurs, la planification de l'approvisionnement.
   programmes de ning et programmes de prévision.
    - **Mon accès :** met en évidence les plans d'approvisionnement auxquels vous avez accès, y compris les plans d'approvisionnement téléchargés.
    - **Lien ERP :** Affiche le nombre d'envois liés par domaine et par programmes téléchargés.

![Dashboard Ticker](/img/media/image78.png)

_Ticker du tableau de bord_
...
_Section Présentation_

  **Sélection de programmes :**
  - Les programmes peuvent être basés sur le serveur ou téléchargés, mais pas les deux
  - Sélection multiple disponible pour les programmes
  ...
  - Pour les programmes serveur, certaines colonnes/fonctionnalités (par exemple, la colonne d'action et les liens QPL) ne sont pas affichées
  - *Remarque : les versions téléchargées et celles du serveur peuvent afficher des données différentes, si les éléments ont été mis à jour sur la version locale et non téléchargés, ou si le QPL a été recalculé en fonction de la date d'aujourd'hui.*
  
  **Caractéristiques du tableau de présentation**
...
3. **Program Spotlight** – Cette section fournit des informations détaillées sur les indicateurs clés d'un seul programme. Assurez-vous d'utiliser les info-bulles et les liens intégrés pour une navigation et une exploration des données efficaces. Les 5 sections ont une info-bulle à côté de l'en-tête et chaque en-tête est lié à un écran QAT auquel vous pouvez accéder en cliquant sur l'en-tête.
  ...
  **Période du rapport :**
  - Affecte uniquement les sections Expéditions, Erreur de prévision et Expirations. La section Score global du plan d'approvisionnement et Qualité des données sont basées sur les calculs QPL et la section État des stocks se situe toujours dans 18 mois dans le futur (y compris le mois en cours).
  - **Programmes serveur :** Par défaut, 6 mois passés et 18 mois futurs (modifiables)
  - **Programmes téléchargés :** Par défaut, 6 mois passés et 18 mois futurs (non modifiables sur le tableau de bord). Les administrateurs peuvent définir des paramètres par défaut personnalisés par programme via l'écran Mettre à jour les informations sur le programme.

![Dashboard Default Report Period](/img/media/image81.png)

_Période de rapport par défaut du tableau de bord dans les paramètres du programme_

  **Mesures Spotlight**
  
  **Note globale du plan d'approvisionnement**
  - Le grand cadran affiche le score global du plan d'approvisionnement pour un programme spécifique. Le score global du plan d’approvisionnement est une moyenne du score de qualité et du score d’état des stocks. En général, des scores plus élevés indiquent une meilleure performance du plan d’approvisionnement. Pour plus d'informations sur la manière dont les scores sont calculés, consultez le rapport Fiche d'évaluation du plan d'approvisionnement.
  ...
  **État des stocks :**
  ...
  - Affiche le score d'état des stocks, basé sur le nombre de mois (18 mois à l'avenir) pendant lesquels chaque unité de planification est approvisionnée conformément au plan.

![Stock Status Section](/img/media/image82.png)

_Section État des stocks_

  **Qualité des données :**
  - Reflète les métriques de la liste des problèmes QAT, indépendamment de la période du rapport.
  ...
  - Affiche le niveau de qualité des données, qui est une moyenne du pourcentage de conformité du plan d'approvisionnement selon les quatre cadrans de qualité des données.
  - Compose pour :
...
_Section d'erreur de prévision_
  ...
  **Expéditions :**
  ...
  - Passez la souris sur le diagramme circulaire pour connaître la répartition des coûts. La ventilation des coûts utilise la valeur totale des expéditions au cours de cette période de rapport comme dénominateur.
  - Le tableau ci-joint montre le nombre d'expéditions dont le financement est à déterminer au cours de la période du rapport.

![Shipment Section](/img/media/image84.png)

_Section Expédition_
