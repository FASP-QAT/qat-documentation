...
export default function DownloadPdfButton({ label, showEverywhere = false, className }) {
  ...
  // Detection logic
  let isUserModule = showEverywhere;
  if (!isUserModule) {
    try {
  ...
  const labels = {
    en: { idle: label || 'Download User Manual', busy: 'Generating...' },
    fr: { idle: label || 'Télécharger le Manuel Utilisateur', busy: 'Génération...' },
    es: { idle: label || 'Descargar Manual de Usuario', busy: 'Generando...' },
  ...
  // If used as a custom component on landing page, it might just be the button
  return (
    <div className={!className ? "no-print" : ""} style={!className ? { 
      ...
      <button
        ...
        className={className || "button button--outline button--primary button--sm"}
        style={!className ? {
          display: 'inline-flex',
          alignItems: 'center',
          gap: '10px',
      ...
      >
        {isGenerating ? (
          ...
          <svg width="16" height="16" viewBox="0 0 512 512" fill={className ? "white" : "#e74c3c"}>
        ...
        <span style={className ? {marginLeft: '10px'} : {}}>{isGenerating ? currentLabels.busy : currentLabels.idle}</span>
      </button>
    </div>
  );
}
